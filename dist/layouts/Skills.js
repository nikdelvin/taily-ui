...
export function Skills(config) {
    ...
    return `
                    ...
                    <div class="flex h-auto w-full flex-col items-center justify-start p-16 max-sm:p-6 max-sm:py-10 max-md:p-8 max-md:py-12 min-[1024px]:max-[1439px]:py-12 min-[1024px]:max-[1439px]:pb-16">
                        ${config.rows.map((row, rowIndex) => (
                            `<div class="flex h-auto w-full flex-col items-${rowIndex % 2 === 0 ? "start" : "end"} justify-start mt-[-8px]">
                                ...
                                <div class="relative w-full grid grid-flow-row content-start gap-6 max-sm:gap-4 grid-cols-[repeat(15,_minmax(0,_1fr))] min-[1024px]:max-[1439px]:grid-cols-10 max-lg:grid-cols-6 max-md:grid-cols-6 max-sm:grid-cols-5 pb-16 max-sm:pb-10 max-md:pb-12 min-[1024px]:max-[1439px]:pb-12">
                                    ${row.skills.map((text, index) => (
                                        `<div
                                            id="skills_${rowIndex}"
                                            class="group card z-[2] !max-h-[65px] !min-h-[65px] !min-w-[65px] max-sm:!min-w-[50px] !flex-row !p-2 ${index % 15 === 7 ? colorsConfig[config.color].cardBGMiddle : ''}"
                        ...
                        <div class="flex h-auto w-full flex-col items-${config.rows.length % 2 === 0 ? "start" : "end"} justify-start max-[1439px]:hidden">
                            ...
                            <div class="relative w-full grid grid-flow-row grid-cols-3 gap-6 lg:grid-cols-5">
                                ${config.music.map((link, index) => (
                                    `<div
                                        class="card z-[2] !max-h-[156px] !min-h-[156px] !min-w-[200px] !p-[2px] ${index % 5 === 2 ? colorsConfig[config.color].cardBGMiddle : ''}"
                                        ${config.shadow != null ? 'data-shadow' : ''}
        ...
        <div id="data-hint" class="absolute top-0 left-0 w-auto z-[4] justify-center hidden pointer-events-none">
            <div class="rounded-3xl pb-[5px] bg-zinc-950/75 w-auto px-[12px] mx-auto text-center">
        ...
        if (window.innerWidth < 1440) {
            ...
            skillsRows.forEach((row, index) => {
                document.querySelectorAll("#skills_"+String(index)).forEach((skill) => {
                    ...
                    skill.className = "group card z-[2] !max-h-[65px] !min-h-[65px] !min-w-[65px] max-sm:!min-w-[50px] !flex-row !p-2"
